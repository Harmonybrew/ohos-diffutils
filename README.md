...
本项目为 OpenHarmony 平台编译了 diffutils，并发布预构建包。

## 获取预构建包

前往 [release 页面](https://github.com/Harmonybrew/ohos-diffutils/releases) 获取。
...
示例：
```sh
hdc file send diffutils-3.12-ohos-arm64.tar.gz /data
hdc shell

cd /data
tar -zxf diffutils-3.12-ohos-arm64.tar.gz
export PATH=$PATH:/data/diffutils-3.12-ohos-arm64/bin
...
cd /root
curl -L -O https://github.com/Harmonybrew/ohos-diffutils/releases/download/3.12/diffutils-3.12-ohos-arm64.tar.gz
tar -zxf diffutils-3.12-ohos-arm64.tar.gz -C /opt
export PATH=$PATH:/opt/diffutils-3.12-ohos-arm64/bin
...
需要用一台 Linux x64 服务器来运行项目里的 build.sh，以实现 diffutils 的交叉编译。
